...
const Navbar = () => {
  ...
  const cargarNotificaciones = async () => {
    try {
      ...
      const noLeidas = response.data.notificaciones.filter(notif => !notif.leido).length;
  ...
  const getTipoNotificacion = (mensaje) => {
    if (mensaje.includes('aprobada') || mensaje.includes('Aprobada')) return 'aprobada';
    if (mensaje.includes('rechazada') || mensaje.includes('Rechazada')) return 'rechazada';
    if (mensaje.includes('cancelada') || mensaje.includes('Cancelada')) return 'cancelada';
    if (mensaje.includes('modificaciones') || mensaje.includes('modificación')) return 'modificacion';
                  ...
                  <div className="notificaciones-dropdown">
                    ...
                    <div className="notificaciones-list">
                      {loadingNotificaciones ? (
                        <div className="notificacion-loading">Cargando notificaciones...</div>
                        ...
                        notificaciones.map(notificacion => {
                          const { mensajePrincipal, comentarioAdmin } = parsearNotificacion(notificacion.mensaje);
                          const tipo = getTipoNotificacion(notificacion.mensaje);
                          
                          return (
                            <div 
                              key={notificacion.notificacion_id}
                              className={`notificacion-item ${notificacion.leido ? 'leida' : 'no-leida'} ${tipo}`}
                              ...
                              <div className="notificacion-contenido">
                                ...
                                {comentarioAdmin && (
                                  <div className="notificacion-comentario">
                                    <strong>Comentario:</strong> {comentarioAdmin}
                                  </div>
                                )}
